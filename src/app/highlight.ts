...
*/
  ...
  // It is only possible in Unicode-aware more using 'u' flag, but we cannot modify highlight.js
  const _NAME_START =
  ...
  content-char    = %x01-08        ; omit NULL (%x00), HTAB (%x09) and LF (%x0A)
                  ...
                  / %x41-5B        ; omit \ (%x5C)
                  / %x5D-7A        ; omit { | } (%x7B-7D)
                  / %x7E-27FF      ; omit IDEOGRAPHIC SPACE (%x3000)
                  / %x3001-D7FF    ; omit surrogates
                  / %xE000-10FFFF
  */
  // eslint-disable-next-line no-control-regex
  const _CONTENT_CHAR = /[\x01-\x08\x0b-\x0c\x0e-\x1f\x21-\x2d\x2f-\x3f\x41-\x5b\x5d-\x7a\x7e-\u27ff\u3001-\ud7ff\ue000-\uffff]/
      ...
      },
      {
        ...
        contains: [
          VARIABLE_MODE,
          {
            ...
            contains: [QUOTED_PATTERN, { scope: 'key', match: KEY }],
          },
        ],
      },
      {
